...
test_reference_params(void)
{
    ...
    wbuf = (href_t *)HDcalloc(MAX(sizeof(unsigned), sizeof(href_t)), SPACE1_DIM1);
    rbuf = (href_t *)HDcalloc(MAX(sizeof(unsigned), sizeof(href_t)), SPACE1_DIM1);
    tbuf = (href_t *)HDcalloc(MAX(sizeof(unsigned), sizeof(href_t)), SPACE1_DIM1);
...
test_reference_obj(void)
{
    ...
    ret = H5Rget_obj_type3((const href_t *)&wbuf[0], H5P_DEFAULT, &obj_type);
    ...
    CHECK(ret, FAIL, "H5Rcreate_object");
    ret = H5Rget_obj_type3((const href_t *)&wbuf[1], H5P_DEFAULT, &obj_type);
    CHECK(ret, FAIL, "H5Rget_obj_type3");
    VERIFY(obj_type, H5O_TYPE_DATASET, "H5Rget_obj_type3");
    ...
    CHECK(ret, FAIL, "H5Rcreate_object");
    ret = H5Rget_obj_type3((const href_t *)&wbuf[2], H5P_DEFAULT, &obj_type);
    CHECK(ret, FAIL, "H5Rget_obj_type3");
    ...
    CHECK(ret, FAIL, "H5Rcreate_object");
    ret = H5Rget_obj_type3((const href_t *)&wbuf[3], H5P_DEFAULT, &obj_type);
    CHECK(ret, FAIL, "H5Rget_obj_type3");
    ...
    /* Open dataset object */
    dset2 = H5Ropen_object((const href_t *)&rbuf[0], dapl_id);
    ...
    group = H5Ropen_object((const href_t *)&rbuf[2], H5P_DEFAULT);
    ...
    tid1 = H5Ropen_object((const href_t *)&rbuf[3], H5P_DEFAULT);
...
test_reference_region(H5F_libver_t libver_low, H5F_libver_t libver_high)
{
    ...
    href_t *wbuf,                   /* buffer to write to disk */
           *rbuf;                   /* buffer read from disk */
//    href_t  nvrbuf[3]={{0},{101},{255}}; /* buffer with non-valid refs */
    ...
    href_t wdata_NA[1],             /* Write buffer */
    ...
    rbuf = (href_t *)HDcalloc(sizeof(href_t), SPACE1_DIM1);
    ...
    CHECK(ret, FAIL, "H5Sselect_elements");
    ret = H5Rcreate_region(fid1, "/Dataset1", sid2, &wdata_NA[0]);
    CHECK(ret, FAIL, "H5Rcreate_region");
     ...
     */
    // Currently fails in assert
//    H5E_BEGIN_TRY {
//      dset2 = H5Ropen_object(&rdata_NA[0], H5P_DEFAULT);
//    } H5E_END_TRY;
//    VERIFY(dset2, H5I_INVALID_HID, "H5Ropen_object");
    ...
    if(libver_high < H5F_LIBVER_V110)
        ...
        ret = H5Rget_obj_type3((const href_t *)&rbuf[0], H5P_DEFAULT, &obj_type);
        CHECK(ret, FAIL, "H5Rget_obj_type3");
        VERIFY(obj_type, H5O_TYPE_DATASET, "H5Rget_obj_type3");
        ...
        sid2 = H5Ropen_region((const href_t *)&rbuf[0]);
        ...
        sid2 = H5Ropen_region((const href_t *)&rbuf[1]);
        CHECK(sid2, H5I_INVALID_HID, "H5Ropen_region");
        ...
        sid2 = H5Ropen_region((const href_t *)&rbuf[2]);
        CHECK(sid2, H5I_INVALID_HID, "H5Ropen_region");
        ...
        //    for(j = 0; j < 3; j++) {
        //        H5E_BEGIN_TRY {
        //            ret = H5Rget_obj_type3(&nvrbuf[j], &obj_type);
        //        } H5E_END_TRY;
...
test_reference_region_1D(H5F_libver_t libver_low, H5F_libver_t libver_high)
{
    ...
    rbuf = (href_t *)HDcalloc(sizeof(href_t), (size_t)SPACE1_DIM1);
    ...
    /* Try to open objects */
    dset3 = H5Ropen_object((const href_t *)&rbuf[0], dapl_id);
    ...
    /* Get the hyperslab selection */
    sid3 = H5Ropen_region((const href_t *)&rbuf[0]);
    ...
    /* Get the element selection */
    sid3 = H5Ropen_region((const href_t *)&rbuf[1]);
    CHECK(sid3, H5I_INVALID_HID, "H5Ropen_region");
...
test_reference_obj_deleted(void)
{
    ...
    hid_t       dataset,    /* Dataset ID               */
                dset2;      /* Dereferenced dataset ID  */
    hid_t       sid1;       /* Dataspace ID             */
    href_t      oref;       /* Object Reference to test */
    H5O_type_t  obj_type;   /* Object type              */
    herr_t      ret;        /* Generic return value     */
    ...
    CHECK(ret, FAIL, "H5Rcreate_object");
    ret = H5Rget_obj_type3((const href_t *)&oref, H5P_DEFAULT, &obj_type);
    CHECK(ret, FAIL, "H5Rget_obj_type3");
    VERIFY(obj_type, H5O_TYPE_DATASET, "H5Rget_obj_type3");
    ...
    dset2 = H5Ropen_object((const href_t *)&oref, H5P_DEFAULT);
    VERIFY(dset2, H5I_INVALID_HID, "H5Ropen_object");
...
test_reference_group(void)
{
    ...
    gid = H5Ropen_object((const href_t *)&rref, H5P_DEFAULT);
...
test_reference_attr(void)
{
    ...
    ret = H5Rget_obj_type3((const href_t *)&ref_wbuf[0], H5P_DEFAULT, &obj_type);
    CHECK(ret, FAIL, "H5Rget_obj_type3");
    VERIFY(obj_type, H5O_TYPE_DATASET, "H5Rget_obj_type3");
    ...
    CHECK(ret, FAIL, "H5Rcreate_attr");
    ret = H5Rget_obj_type3((const href_t *)&ref_wbuf[1], H5P_DEFAULT, &obj_type);
    CHECK(ret, FAIL, "H5Rget_obj_type3");
    VERIFY(obj_type, H5O_TYPE_DATASET, "H5Rget_obj_type3");
    ...
    CHECK(ret, FAIL, "H5Rcreate_attr");
    ret = H5Rget_obj_type3((const href_t *)&ref_wbuf[2], H5P_DEFAULT, &obj_type);
    CHECK(ret, FAIL, "H5Rget_obj_type3");
    VERIFY(obj_type, H5O_TYPE_GROUP, "H5Rget_obj_type3");
    ...
    CHECK(ret, FAIL, "H5Rcreate_attr");
    ret = H5Rget_obj_type3((const href_t *)&ref_wbuf[3], H5P_DEFAULT, &obj_type);
    CHECK(ret, FAIL, "H5Rget_obj_type3");
    VERIFY(obj_type, H5O_TYPE_NAMED_DATATYPE, "H5Rget_obj_type3");
...
test_reference_compat_conv(void)
{
    ...
    href_t     *rbuf_obj;       /* Buffer read from disk    */
    hdset_reg_ref_t  *wbuf_reg; /* Buffer to write to disk  */
    href_t           *rbuf_reg; /* Buffer read from disk    */
    H5O_type_t       obj_type;  /* Object type              */
    herr_t  ret;                /* Generic return value     */
    ...
    rbuf_obj = (href_t *)HDcalloc(sizeof(href_t), SPACE1_DIM1);
    wbuf_reg = (hdset_reg_ref_t *)HDcalloc(sizeof(hdset_reg_ref_t), 2);
    rbuf_reg = (href_t *)HDcalloc(sizeof(href_t), 2);
    ...
    ret = H5Rget_obj_type3((const href_t *)&rbuf_obj[0], H5P_DEFAULT, &obj_type);
    CHECK(ret, FAIL, "H5Rget_obj_type3");
    VERIFY(obj_type, H5O_TYPE_DATASET, "H5Rget_obj_type3");

    ret = H5Rget_obj_type3((const href_t *)&rbuf_obj[1], H5P_DEFAULT, &obj_type);
    CHECK(ret, FAIL, "H5Rget_obj_type3");
    VERIFY(obj_type, H5O_TYPE_DATASET, "H5Rget_obj_type3");

    ret = H5Rget_obj_type3((const href_t *)&rbuf_obj[2], H5P_DEFAULT, &obj_type);
    CHECK(ret, FAIL, "H5Rget_obj_type3");
    VERIFY(obj_type, H5O_TYPE_GROUP, "H5Rget_obj_type3");

    ret = H5Rget_obj_type3((const href_t *)&rbuf_obj[3], H5P_DEFAULT, &obj_type);
    CHECK(ret, FAIL, "H5Rget_obj_type3");
    VERIFY(obj_type, H5O_TYPE_NAMED_DATATYPE, "H5Rget_obj_type3");


    /* Make sure the referenced objects can be opened */
    dset2 = H5Ropen_object((const href_t *)&rbuf_obj[0], H5P_DEFAULT);
    CHECK(dset2, H5I_INVALID_HID, "H5Ropen_object");

    ret = H5Dclose(dset2);
    CHECK(ret, FAIL, "H5Dclose");

    dset2 = H5Ropen_object((const href_t *)&rbuf_obj[1], H5P_DEFAULT);
    CHECK(dset2, H5I_INVALID_HID, "H5Ropen_object");

    ret = H5Dclose(dset2);
    CHECK(ret, FAIL, "H5Dclose");

    group2 = H5Ropen_object((const href_t *)&rbuf_obj[2], H5P_DEFAULT);
    CHECK(group2, H5I_INVALID_HID, "H5Ropen_object");

    ret = H5Gclose(group2);
    CHECK(ret, FAIL, "H5Gclose");

    tid2 = H5Ropen_object((const href_t *)&rbuf_obj[3], H5P_DEFAULT);
    ...
    /* Verify type of objects pointed at */
    ret = H5Rget_obj_type3((const href_t *)&rbuf_reg[0], H5P_DEFAULT, &obj_type);
    CHECK(ret, FAIL, "H5Rget_obj_type3");
    VERIFY(obj_type, H5O_TYPE_DATASET, "H5Rget_obj_type3");

    ret = H5Rget_obj_type3((const href_t *)&rbuf_reg[1], H5P_DEFAULT, &obj_type);
    CHECK(ret, FAIL, "H5Rget_obj_type3");
    VERIFY(obj_type, H5O_TYPE_DATASET, "H5Rget_obj_type3");

    /* Make sure the referenced objects can be opened */
    dset2 = H5Ropen_object((const href_t *)&rbuf_reg[0], H5P_DEFAULT);
    CHECK(dset2, H5I_INVALID_HID, "H5Ropen_object");

    ret = H5Dclose(dset2);
    CHECK(ret, FAIL, "H5Dclose");

    dset2 = H5Ropen_object((const href_t *)&rbuf_reg[1], H5P_DEFAULT);
    CHECK(dset2, H5I_INVALID_HID, "H5Ropen_object");
