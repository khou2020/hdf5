    ...
    const H5D_io_info_t *io_info, const H5D_type_info_t *type_info, const H5D_chunk_map_t *fm)
{
    ...
    if(!chunk_entry->full_overwrite || io_info->op_type == H5D_IO_OP_READ) {
        ...
        t4 = MPI_Wtime();
        if (io_info->op_type == H5D_IO_OP_WRITE){
            eval_add_time(EVAL_TIMER_H5F_block_read_w, t4 - t3);
        }
        else{
            eval_add_time(EVAL_TIMER_H5F_block_read_r, t4 - t3);
        }
    ...
    switch (io_info->op_type) {
        ...
        case H5D_IO_OP_WRITE:
            ...
            t3 = MPI_Wtime();
            eval_add_time(EVAL_TIMER_H5D__filtered_collective_chunk_entry_io_Unpack_w, t3 - t4);
            ...
            t4 = MPI_Wtime();
            eval_add_time(EVAL_TIMER_H5D__filtered_collective_chunk_entry_io_Filter_w, t4 - t3);
