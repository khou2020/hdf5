...
H5Rget_obj_type1(hid_t id, H5R_type_t ref_type, const void *ref)
{
    ...
    if(ref_type == H5R_OBJECT1) {
        ...
        if((file_id = H5F_get_file_id(id, vol_obj_type)) < 0)
            HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, H5G_UNKNOWN, "not a file or file object")
        ...
        if(NULL == (f = (H5F_t *)H5VL_object_data(vol_obj_file)))
            HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, H5G_UNKNOWN, "invalid VOL object")

        /* Get object address */
        if(H5R__decode_addr_region_compat(f, buf, &buf_size, &obj_addr, NULL) < 0)
            HGOTO_ERROR(H5E_REFERENCE, H5E_CANTDECODE, H5G_UNKNOWN, "unable to get object address")
    }
    ...
    if(file_id != H5I_INVALID_HID && H5I_dec_ref(file_id) < 0)
        HDONE_ERROR(H5E_REFERENCE, H5E_CANTDEC, H5G_UNKNOWN, "unable to decrement refcount on file")
...
H5Rdereference1(hid_t obj_id, H5R_type_t ref_type, const void *ref)
{
    ...
    if(ref_type == H5R_OBJECT1) {
        ...
        /* Get the file for the object */
        if((file_id = H5F_get_file_id(obj_id, vol_obj_type)) < 0)
    ...
    size_t size)
{
    ...
    if(ref_type == H5R_OBJECT1) {
        ...
        /* Retrieve file from VOL object */
        if(NULL == (f = (H5F_t *)H5VL_object_data(vol_obj_file)))
            HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, (-1), "invalid VOL object")
    ...
    if(file_id != H5I_INVALID_HID && H5I_dec_ref(file_id) < 0)
        HDONE_ERROR(H5E_REFERENCE, H5E_CANTDEC, (-1), "unable to decrement refcount on file")
    FUNC_LEAVE_API(ret_value)
}   /* end H5Rget_name() */
