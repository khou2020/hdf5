    ...
    size_t buf_stride, size_t bkg_stride, void *buf, void *bkg)
{
    ...
    switch(cdata->command) {
        ...
        case H5T_CONV_CONV:
        {
            ...
            HDassert(src->shared->u.atomic.u.r.getsize);
            HDassert(src->shared->u.atomic.u.r.read);
            HDassert(dst->shared->u.atomic.u.r.write);
            ...
            while(nelmts > 0) {
                ...
                for(elmtno = 0; elmtno < safe; elmtno++) {
                    ...
                    if(0 == (buf_size = src->shared->u.atomic.u.r.getsize(src->shared->u.atomic.u.r.f, s, src->shared->size, dst->shared->u.atomic.u.r.f)))
                    ...
                    /* Read reference */
                    if(src->shared->u.atomic.u.r.read(src->shared->u.atomic.u.r.f, s, src->shared->size, dst->shared->u.atomic.u.r.f, conv_buf, buf_size) < 0)
                        HGOTO_ERROR(H5E_DATATYPE, H5E_READERROR, FAIL, "can't read reference data")

                    /* Write reference to destination location */
                    if(dst->shared->u.atomic.u.r.write(src->shared->u.atomic.u.r.f, conv_buf, buf_size, src->shared->u.atomic.u.r.rtype,
