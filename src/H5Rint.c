...
/* Header size */
#define H5R_ENCODE_HEADER_SIZE  (3 * H5_SIZEOF_UINT8_T + H5_SIZEOF_UINT16_T)

/* Encode macro */
#define H5R_ENCODE(func, val, m) do {                       \
    enc_size = buf_size_left;                               \
    if(func(val, p, &enc_size) < 0)                         \
...
/* Decode macro */
#define H5R_DECODE(func, val, m) do {                       \
    dec_size = buf_size_left;                               \
    if(func(p, &dec_size, val) < 0)                         \
...
static herr_t H5R__encode_obj_addr(haddr_t obj_addr, unsigned char *buf, size_t *nalloc);
static herr_t H5R__decode_obj_addr(const unsigned char *buf, size_t *nbytes, haddr_t *obj_addr_ptr);
 ...
 */
herr_t
H5R__create_object(const char *filename, haddr_t obj_addr, struct href *ref)
{
    ...
    if(NULL == (ref->ref.obj.filename = HDstrdup(filename)))
        HGOTO_ERROR(H5E_REFERENCE, H5E_CANTCOPY, FAIL, "Cannot copy file name")
    ref->ref.obj.addr = obj_addr;

    /* Size will be cached when encoding is needed */
    ref->encode_size = 0;

    H5R_LOG_DEBUG("Created object reference, %d, filename=%s, obj_addr=%zu, encode size=%zu",
        (int)sizeof(struct href), ref->ref.obj.filename, ref->ref.obj.addr, ref->encode_size);
 ...
 */
herr_t
H5R__create_region(const char *filename, haddr_t obj_addr, H5S_t *space,
    struct href *ref)
{
    herr_t ret_value = SUCCEED; /* Return value */
    ...
    H5R_LOG_DEBUG("Created region reference, %d, filename=%s, obj_addr=%zu, encode size=%zu",
        (int)sizeof(struct href), ref->ref.obj.filename, ref->ref.obj.addr, ref->encode_size);
 ...
 */
herr_t
H5R__create_attr(const char *filename, haddr_t obj_addr, const char *attr_name,
    struct href *ref)
{
    herr_t ret_value = SUCCEED; /* Return value */
    ...
    H5R_LOG_DEBUG("Created attribute reference, %d, filename=%s, obj_addr=%zu, attr name=%s, encode size=%zu",
        (int)sizeof(struct href), ref->ref.obj.filename, ref->ref.obj.addr, ref->ref.attr.name, ref->encode_size);
 ...
 */
hid_t
H5R__reopen_file(struct href *ref, hid_t fapl_id)
{
    void *new_file = NULL;
     ...
     */
    /* TODO Should allow for other VOL IDs */
    ...
    if(H5R__set_loc_id((struct href *)ref, ret_value) < 0)
...
H5R__equal(const struct href *ref1, const struct href *ref2)
{
    ...
    /* Compare filenames */
    if(0 != HDstrcmp(ref1->ref.obj.filename, ref2->ref.obj.filename))
        HGOTO_DONE(FALSE);
...
H5R__copy(const struct href *src_ref, struct href *dest_ref)
{
    ...
    if(NULL == (dest_ref->ref.obj.filename = HDstrdup(src_ref->ref.obj.filename)))
    ...
    dest_ref->encode_size = src_ref->encode_size;
...
H5R__get_file_name(const struct href *ref, char *name, size_t size)
{
    ...
    if (name) {
        ...
        HDmemcpy(name, ref->ref.obj.filename, copy_len);
        name[copy_len] = '\0';
    }
    ...
    unsigned flags)
{
     ...
     * - Protocol version
     * - Reference type
     * - Additional flags (unused): UTF-8 encoding, fixed size reference
     * - Reference (specific)
     *   + Dataspace
     *   + Attribute name
     * - File name
    ...
    if(buf && *nalloc >= buf_size) {
        ...
        /* Sanity check */
        HDcompile_assert(H5_VOL_MAX <= UINT16_MAX);

        /* Get the VOL object */
        if(NULL == (vol_obj = H5VL_vol_object(ref->loc_id)))
        ...
        *p++ = (uint8_t)H5R_ENCODE_VERSION;
        ...
        /* Encode the VOL ID */
        UINT16ENCODE(p, vol_obj->connector->cls->value);
    ...
    /* Encode object address */
    H5R_ENCODE(H5R__encode_obj_addr, ref->ref.obj.addr, "Cannot encode object address");

    if(flags & H5R_IS_EXTERNAL) {
        /* Encode file name */
        H5R_ENCODE(H5R__encode_string, ref->ref.obj.filename, "Cannot encode filename");
    ...
    switch(ref->type) {
        ...
        case H5R_DATASET_REGION2:
            /* Encode dataspace */
            H5R_ENCODE(H5R__encode_region, ref->ref.reg.space,
                "Cannot encode region");
            break;
        case H5R_ATTR:
            /* Encode attribute name */
            H5R_ENCODE(H5R__encode_string, ref->ref.attr.name,
 ...
 */
herr_t
H5R__decode(const char *filename, const unsigned char *buf, size_t *nbytes,
    struct href *ref)
{
    const uint8_t *p = (const uint8_t *)buf;
    size_t buf_size = 0, buf_size_left, dec_size = 0;
    uint16_t vol_id;
    uint8_t flags;
    herr_t ret_value = SUCCEED;
    ...
    HDassert(nbytes);
    HDassert(ref);
    buf_size_left = *nbytes;

    /* Don't decode if buffer size isn't big enough */
    if(buf_size_left < H5R_ENCODE_HEADER_SIZE)
    ...
    buf_size_left -= H5R_ENCODE_HEADER_SIZE;
    buf_size += H5R_ENCODE_HEADER_SIZE;

    /* Decode object address */
    H5R_DECODE(H5R__decode_obj_addr, &ref->ref.obj.addr, "Cannot decode object address");

    if(flags & H5R_IS_EXTERNAL) {
        /* Decode file name */
        H5R_DECODE(H5R__decode_string, &ref->ref.obj.filename, "Cannot decode filename");

        /* TODO Decode VOL info */
    } else {
        if(NULL == (ref->ref.obj.filename = HDstrdup(filename)))
            HGOTO_ERROR(H5E_REFERENCE, H5E_CANTCOPY, FAIL, "Cannot copy file name")
    }
    ...
    switch(ref->type) {
        ...
        case H5R_DATASET_REGION2:
            /* Decode dataspace */
            H5R_DECODE(H5R__decode_region, &ref->ref.reg.space, "Cannot decode region");
            break;
        case H5R_ATTR:
            /* Decode attribute name */
            H5R_DECODE(H5R__decode_string, &ref->ref.attr.name, "Cannot decode attribute name");
            break;
        case H5R_OBJECT1:
        case H5R_DATASET_REGION1:
        case H5R_BADTYPE:
